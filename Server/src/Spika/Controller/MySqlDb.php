...
// 		}
	...
	public function getChatMembers(Application $app, $chat_id){
		...
		$sql = "SELECT chat_member.user_id, chat_member.chat_id, user.firstname, user.lastname, user.image, user.image_thumb, user.android_push_token, user.ios_push_token FROM chat_member, user WHERE user.id = chat_member.user_id AND chat_member.chat_id = ? ";
	...
	public function updateChatMember($app, $chat_id, $user_id, $values){
	...
	public function createMessage(Application $app, $chat_id, $user_id, $type, $text, $file_id, $thumb_id, $longitude, $latitude, $root_id, $parent_id){
	...
	public function getChildMessages(Application $app, $child_id_list){
		...
		$sql = "SELECT message.*, user.firstname, user.lastname, user.image, user.image_thumb FROM message, user WHERE message.user_id = user.id and message.id IN (" . $child_id_list . ") ORDER BY message.id DESC";
	...
	public function getRecentAllChats(Application $app, $user_id, $offset){
		...
		$sql = "SELECT chat_member.chat_id, chat_member.unread, chat.name AS chat_name, chat.image AS image_thumb, chat.modified, chat.type, chat.group_id, chat.seen_by FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 ORDER BY chat.modified DESC LIMIT " . $offset . ", " . RECENT_PAGE_SIZE;
	...
	public function getCountRecentAllChats(Application $app, $user_id){
		...
		$sql = "SELECT COUNT(*) FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 ORDER BY chat.modified DESC";
	...
	public function getRecentGroupChats(Application $app, $user_id, $offset){
		...
		$sql = "SELECT chat_member.chat_id, chat_member.unread, chat.name AS chat_name, chat.image AS image_thumb, chat.modified, chat.type, chat.group_id, chat.seen_by FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 AND (chat.type = 2 OR chat.type = 3) ORDER BY chat.modified DESC LIMIT " . $offset . ", " . RECENT_PAGE_SIZE;
	...
	public function getCountRecentGroupChats(Application $app, $user_id){
		...
		$sql = "SELECT COUNT(*) FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 AND (chat.type = 2 OR chat.type = 3) ORDER BY chat.modified DESC";
	...
	public function getRecentPrivateChats(Application $app, $user_id, $offset){
		...
		$sql = "SELECT chat_member.chat_id, chat_member.unread, chat.name AS chat_name, chat.image AS image_thumb, chat.modified, chat.type, chat.group_id, chat.seen_by FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 AND chat.type = 1 ORDER BY chat.modified DESC LIMIT " . $offset . ", " . RECENT_PAGE_SIZE;
	...
	public function getCountRecentPrivateChats(Application $app, $user_id){
		...
		$sql = "SELECT COUNT(*) FROM chat_member, chat WHERE chat_member.chat_id = chat.id AND chat_member.user_id = ? AND chat.has_messages = 1 AND chat.type = 1 ORDER BY chat.modified DESC";
